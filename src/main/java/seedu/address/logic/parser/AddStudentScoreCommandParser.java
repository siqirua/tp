...
public class AddStudentScoreCommandParser implements Parser<AddStudentScoreCommand> {
    ...
    public AddStudentScoreCommand parse(String args) throws ParseException {
        ...
        ArgumentMultimap argMultimap =
                ArgumentTokenizer.tokenize(args, PREFIX_STUDENT_ID, PREFIX_COMPONENT_NAME, PREFIX_MARKS, PREFIX_COMMENT);

        if (!arePrefixesPresent(argMultimap, PREFIX_STUDENT_ID, PREFIX_COMPONENT_NAME, PREFIX_MARKS, PREFIX_COMMENT)
                || !argMultimap.getPreamble().isEmpty()) {
            throw new ParseException(String.format(MESSAGE_INVALID_COMMAND_FORMAT, AddStudentCommand.MESSAGE_USAGE));
        }

        argMultimap.verifyNoDuplicatePrefixesFor(PREFIX_STUDENT_ID, PREFIX_COMPONENT_NAME, PREFIX_MARKS, PREFIX_COMMENT);
