...
public class TestGcDataUtil {
    ...
    public static GradedComponent[] getSampleGradedComponentsAfterAdding() {
        return new GradedComponent[]{new GradedComponent(new GcName("Midterm"), new MaxMarks(50), new Weightage(30)),
                new GradedComponent(new GcName("Final"), new MaxMarks(100), new Weightage(40)),
        new GradedComponent(new GcName("Practical Exam"), new MaxMarks(30), new Weightage(30))};
    }

    public static GradedComponent[] getSampleGradedComponentsAfterDeleting() {
        return new GradedComponent[]{new GradedComponent(new GcName("Midterm"), new MaxMarks(50), new Weightage(30))};
    }

    public static ReadOnlyGradedComponentBook getSampleGcBook(String selectedGc) {
        GradedComponentBook sampleAb = new GradedComponentBook();
        GradedComponent[] gcToBeAdded;
        switch (selectedGc) {
        case "add":
            gcToBeAdded = getSampleGradedComponentsAfterAdding();
            break;
        case "delete":
            gcToBeAdded = getSampleGradedComponentsAfterDeleting();
            break;
        case "create":
            gcToBeAdded = getSampleGradedComponents();
            break;
        default:
            gcToBeAdded = new GradedComponent[]{};
        }
        for (GradedComponent sampleGradedComponent : gcToBeAdded) {
